...
public class Space_001PlacementHandler {
    ...
    private static void spawnVoidPlatform(World world, BlockPos pos) {
        if (world.getBlockState(pos).getBlock() != BSR.PORTAL_BLOCK) {
            BlockState platformBlock = BSR.randomTerracotta();
            for (int x = -3; x < 4; x++) {
                for (int z = -3; z < 4; z++) {
            ...
            world.setBlockState(pos, BSR.PORTAL_BLOCK.getDefaultState());
