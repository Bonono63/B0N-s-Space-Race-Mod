...
import net.MrBonono63.bsr.util.noise.OctaveOpenSimplexNoise;
...
public class AsteroidFeature extends Feature<DefaultFeatureConfig> {
    ...
    private static final BlockState ROCK = BSRBlocks.CARBONATE_ROCK.getDefaultState();
    private static final BlockState IRON = BSRBlocks.ASTEROID_IRON.getDefaultState();
    ...
    private boolean generate(IWorld world, Random rand, BlockPos pos) {
        ...
        int baseHeight = rand.nextInt(6) + 14;
        ...
        for (int quarter = 0; quarter < 4; ++quarter) {
            for (int localPosition = -1; localPosition < 2; ++localPosition) {
                ...
                generateAsteroid(world, rand, posMutable, posMutable2, baseHeight + rand.nextInt(3), lowY);
            }
        }
        return true;
    }

    private void generateAsteroid(ModifiableWorld world, Random rand, final BlockPos centre, BlockPos.Mutable mutable, int height, int lowY) {
        ...
        for (int xOffset = -3; xOffset < 4; ++xOffset) {
            ...
            for (int zOffset = -3; zOffset < 4; ++zOffset) {
                ...
                for (int y = lowY - 5; y < localHeight + 1; ++y) {
                    mutable.setY(y);
                    world.setBlockState(mutable, rand.nextInt(3) == 0 ? ROCK : IRON, 19);
                }
            }
        }
    }
